# ruff: noqa: E501
"""Default templates for the Project Health agents.

Templates use ``string.Template`` (``$placeholder`` syntax) so they do not
collide with the literal ``{`` / ``}`` braces that appear in JSON examples
inside the prompts.

Placeholder vocabulary across prompts:
- ``$language_instructions`` — facilitator-style language instruction
- ``$report_language_instructions`` — report-style language instruction
- ``$obt_assumptions`` — OBT_INTERNAL_ASSUMPTIONS block
- ``$obt_domain_guidance`` — OBT_DOMAIN_GUIDANCE block
- ``$required_opening_details`` — REQUIRED_OPENING_DETAILS block
- ``$domain_list`` — localized list of internal coverage domains
- ``$coverage_hints`` — runtime coverage hint string
- ``$language`` — interview language code (e.g. "pt")
"""
...
_FACILITATOR_TEMPLATE = """You are a warm, respectful facilitator for Oral Bible Translation project health conversations.
...
_COVERAGE_PLANNER_TEMPLATE = """You are a hidden Coverage Planner for an Oral Bible Translation project health interview.
...
_EVIDENCE_MAPPER_TEMPLATE = """You are a hidden Evidence Mapper agent for Oral Bible Translation project health interviews.
...
_SCORING_TEMPLATE = """You are a hidden Scoring agent for Oral Bible Translation project health interviews.
...
_INTERVIEW_CONTEXT_TEMPLATE = """You extract interview context from an Oral Bible Translation interview transcript.
...
_TEAM_REPORT_TEMPLATE = """You are a Report Writer creating a team-facing project health report for an Oral Bible Translation team. $report_language_instructions
...
_ADMIN_REPORT_TEMPLATE = """You are a Report Writer creating an admin-facing OBT project health report.
...
_GUARDRAIL_TEMPLATE = """You are a Guardrail agent. Review the facilitator's proposed response and check for violations.
...
_DEFAULTS: dict[str, DefaultPrompt] = {
    "facilitator_system": {
        "name": "Facilitator system prompt",
        "description": "Main facilitator persona used for every team turn. Controls tone, conversation style, and the closing handoff to the Finish button.",
        "template": _FACILITATOR_TEMPLATE,
        "placeholders": (
    ...
    "coverage_planner": {
        ...
        "description": "Hidden agent that decides which domains have been touched and when the conversation should move to closing.",
        "template": _COVERAGE_PLANNER_TEMPLATE,
        "placeholders": ("obt_domain_guidance", "required_opening_details", "language"),
    },
    "evidence_mapper": {
        "name": "Evidence mapper",
        "description": "Hidden agent that extracts evidence items from each new team turn.",
        "template": _EVIDENCE_MAPPER_TEMPLATE,
        "placeholders": ("obt_domain_guidance",),
    },
    "scoring": {
        "name": "Scoring",
        "description": "Final scoring agent that produces domain scores at the end of the interview.",
        "template": _SCORING_TEMPLATE,
        "placeholders": ("obt_domain_guidance",),
    },
    "interview_context": {
        "name": "Interview context extractor",
        "description": "Extracts participant names, language, team composition from the transcript.",
        "template": _INTERVIEW_CONTEXT_TEMPLATE,
        "placeholders": (),
    },
    "team_report": {
        "name": "Team report writer",
        "description": "Generates the team-facing report shown to the project team after the interview.",
        "template": _TEAM_REPORT_TEMPLATE,
        "placeholders": ("report_language_instructions",),
    },
    "admin_report": {
        "name": "Admin report writer",
        "description": "Generates the supervisor-facing report with sustainability index, risks, and recommended actions.",
        "template": _ADMIN_REPORT_TEMPLATE,
        "placeholders": (),
    },
    "guardrail": {
        "name": "Guardrail",
        "description": "Reviews each proposed facilitator response and flags violations of the conversation style rules.",
...
def get_default(prompt_key: str) -> DefaultPrompt:
    if prompt_key not in _DEFAULTS:
        raise KeyError(prompt_key)
    return _DEFAULTS[prompt_key]


def iter_defaults() -> list[tuple[str, DefaultPrompt]]:
    return [(k, _DEFAULTS[k]) for k in PROMPT_KEYS]


def get_default_template(prompt_key: str) -> str:
    return get_default(prompt_key)["template"]


def get_placeholders(prompt_key: str) -> tuple[str, ...]:
    return get_default(prompt_key)["placeholders"]
