...
@inngest_client.create_function(
    ...
    # No on_failure hook: a failure that reaches this level is infrastructure (the database
    # or the bucket), not an answer's. Per-answer failures are already recorded as `failed`
    # rows by the pass itself, and the retry re-reads whatever is still `pending`.
    retries=3,
)
async def transcribe_session_answers_fn(ctx: inngest.Context, step: inngest.Step) -> str:
    ...
    async def _transcribe() -> str:
        # Its own session: this runs long after the request that asked for it is gone.
