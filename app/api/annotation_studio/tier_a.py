...
from app.services.annotation_studio import tier_a_service

router = APIRouter()


@router.get("/languages/{language_id}/tier-a/words", response_model=list[WordResponse])
async def list_words(language_id: str, db: Db, _: CurrentUser) -> list[WordResponse]:
...
async def update_word(word_id: str, payload: WordUpdate, db: Db, _: CurrentUser) -> WordResponse:
...
async def delete_word(word_id: str, db: Db, _: CurrentUser) -> None:
...
async def complete_reference(word_id: str, db: Db, _: CurrentUser) -> WordResponse:
...
async def clear_reference(word_id: str, db: Db, _: CurrentUser) -> WordResponse:
...
async def list_recordings(language_id: str, db: Db, _: CurrentUser) -> list[TierARecordingResponse]:
...
async def create_recording(
    word_id: str, payload: TierARecordingCreate, db: Db, _: CurrentUser
) -> TierARecordingTicket:
...
async def complete_recording(recording_id: str, db: Db, _: CurrentUser) -> TierARecordingResponse:
