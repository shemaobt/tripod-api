...
async def audio_signed_url(audio_id: str, db: Db, user: CurrentUser) -> AudioUrlResponse:
    ...
    It is NOT the only route in this API that serves these bytes, and it would be a
    dangerous docstring if it claimed otherwise. The Oral Collector's own acousteme
    routes (``GET /api/oc/acoustemes/{audio_id}/audio``, and the collection listing
    beside it) mint the same signed URL for the same private object behind nothing but
    ``get_current_user`` — no app role, no project scoping. Until those are closed, the
    gate below narrows nothing for anyone holding any Tripod account. See ENG-290.
    """
